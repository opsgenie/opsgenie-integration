...
var configParameters = map[string]string{"apiKey": API_KEY,
	...
	"vcsa2opsgenie.logger":"warning",
	"vcsa2opsgenie.http.proxy.enabled" : "false",
	"vcsa2opsgenie.http.proxy.port" : "1111",
	"vcsa2opsgenie.http.proxy.host": "localhost",
	"vcsa2opsgenie.http.proxy.protocol":"http",
	"vcsa2opsgenie.http.proxy.username": "",
	"vcsa2opsgenie.http.proxy.password": ""}
...
func configureLogger ()log.Logger{
	level := configParameters["vcsa2opsgenie.logger"]
	var logFilePath = parameters["logPath"]

	if len(logFilePath) == 0 {
		logFilePath = "/var/log/opsgenie/vcsa2opsgenie.log"
	}
	...
	if err != nil {
		...
		fileTmp, errTmp := os.OpenFile("/tmp/vcsa2opsgenie.log", os.O_CREATE | os.O_WRONLY | os.O_APPEND, 0666)
...
func getHttpClient (timeout int) *http.Client {
	...
	var proxyEnabled = configParameters["vcsa2opsgenie.http.proxy.enabled"]
	var proxyHost = configParameters["vcsa2opsgenie.http.proxy.host"]
	var proxyPort = configParameters["vcsa2opsgenie.http.proxy.port"]
	var scheme = configParameters["vcsa2opsgenie.http.proxy.protocol"]
	var proxyUsername = configParameters["vcsa2opsgenie.http.proxy.username"]
	var proxyPassword = configParameters["vcsa2opsgenie.http.proxy.password"]
...
func http_post()  {
	...
	var logPrefix = "[VCSA2OpsGenie] "

	apiUrl := configParameters["opsgenie.api.url"] + "/v1/json/vcsa"
	...
	for i := 1; i <= 3; i++ {
		...
		if error == nil {
			...
			if err == nil{
				if resp.StatusCode == 200{
					if logger != nil {
						...
						logger.Info(logPrefix +  "Data from VCSA posted to " + target + " successfully")					}
				}else{
					if logger != nil {
						logger.Error(logPrefix + "Couldn't post data from VCSA to " + target + " successfully; Response code: " + strconv.Itoa(resp.StatusCode) + " Response Body: " + string(body[:]))
					}
				}
			}else{
				if logger != nil {
			...
			if logger != nil {
				logger.Error(logPrefix + "Failed to post data from VCSA. ", error)
			}
		}
		if resp != nil{
			defer resp.Body.Close()
